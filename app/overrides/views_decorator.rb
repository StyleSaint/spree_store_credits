...
Deface::Override.new(
  ...
  :text => "<br /><% if check_super_admin_role %><%= link_to_with_icon('add', t('add_store_credit'), new_admin_user_store_credit_url(user)) %><% end %><br /><%= link_to_with_icon('add', t('show_history'), admin_user_store_credits_path(user)) %>",
  :disabled => false)
